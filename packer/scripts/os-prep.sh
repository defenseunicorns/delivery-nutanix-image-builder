...
# If Network Manager is being used configure it to only manage eth0 so it doesn't interfere with CNI: https://docs.cilium.io/en/latest/network/concepts/ipam/eni/#node-configuration
if systemctl list-units --full | grep -Poi "NetworkManager.service" &>/dev/null; then
  # Indent with tabs to prevent spaces in heredoc output
	...
	[keyfile]
	unmanaged-devices=interface-name:cali*;interface-name:flannel*;interface-name:cil*;interface-name:eth*,except:interface-name:eth0
	EOF
